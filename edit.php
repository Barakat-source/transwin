...
$pageTitle = "Modification de Mission";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $db->prepare("UPDATE voyages SET 
            date_voyage = ?, 
            client = ?, 
            chauffeur = ?, 
            matricule = ?, 
        ...
        Logger::log($db, "MODIFICATION", "Mise à jour mission ID #$id ($client)");
        ...
        <div class="space-y-8">
            <div class="card-premium p-8">
                <div class="flex items-center gap-3 mb-8 border-b pb-4 border-slate-100">
                    ...
                    <h3 class="text-xl font-black text-slate-800 tracking-tight">Modifier la Mission #<?= $id ?></h3>
                </div>
                ...
                <form method="POST" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="date" name="date_voyage" required class="input-premium" value="<?= $v['date_voyage'] ?>">
                        </div>
                        <div class="space-y-2">
                            ...
                            <select name="flux_type" class="input-premium">
                                <option value="IMPORT" <?= $v['flux_type'] == 'IMPORT' ? 'selected' : '' ?>>IMPORT</option>
                                <option value="EXPORT" <?= $v['flux_type'] == 'EXPORT' ? 'selected' : '' ?>>EXPORT</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="chauffeur" required placeholder="Nom complet" class="input-premium pl-12" value="<?= htmlspecialchars($v['chauffeur']) ?>">
                            </div>
                        </div>
                        <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="client" required placeholder="Nom du Client" class="input-premium" value="<?= htmlspecialchars($v['client']) ?>">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-black text-slate-400 uppercase tracking-widest px-1">Numéro TC</label>
                            <input type="text" name="tc" placeholder="N° Conteneur" class="input-premium" value="<?= htmlspecialchars($v['tc']) ?>">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="depart" required placeholder="Point A" class="input-premium" value="<?= htmlspecialchars($v['depart']) ?>">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-black text-slate-400 uppercase tracking-widest px-1">Destination</label>
                            <input type="text" name="destination" required placeholder="Point B" class="input-premium" value="<?= htmlspecialchars($v['destination']) ?>">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="remorque_type" placeholder="Ex: 40' HC / 20' ST" class="input-premium" value="<?= htmlspecialchars($v['remorque_type']) ?>">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-black text-slate-400 uppercase tracking-widest px-1">Observations</label>
                            <textarea name="observation" rows="1" placeholder="Notes complémentaires..." class="input-premium"><?= htmlspecialchars($v['observation']) ?></textarea>
                        </div>
                    </div>

                    <div class="pt-6 border-t border-slate-50 flex items-center justify-between">
                        <a href="index.php" class="text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors">Annuler</a>
                        <button type="submit" class="btn-premium px-10 bg-blue-600 hover:bg-blue-700 shadow-blue-600/30">
